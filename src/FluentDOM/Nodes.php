...
namespace FluentDOM {
  ...
  class Nodes implements \ArrayAccess, \Countable, \IteratorAggregate {
    ...
    private ?Loadable $_loaders = NULL;

    /**
     * A callback used to convert the selector to xpath before use
     */
    private ?\Closure $_onPrepareSelector = NULL;

    protected ?Nodes $_parent = NULL;
     ...
     */
    protected bool $_useDocumentContext = TRUE;

    /**
     * Store the loaded content type and options
     */
    private array $_loadingContext = [];

    private ?Serializer\Factory\Group $_serializerFactories = NULL;
     ...
     */
    public function __construct(mixed $source = NULL, string $contentType = NULL) {
      if (NULL !== $source) {
     ...
     * @throws \InvalidArgumentException
     */
    public function loaders(Loadable|iterable $loaders = NULL): Loadable {
      if (NULL !== $loaders) {
     ...
     */
    public function getLoadingOptions(?string $contentType = NULL): mixed {
     ...
     */
    public function item(int $position): \DOMNode|NULL {
    ...
    ): Xpath|\DOMNodeList|float|string {
      if (NULL !== $expression) {
     ...
     * @throws \LogicException
     */
    private function applyNamespaces(): void {
      $document = $this->getDocument();
      if ($document instanceof Document) {
    ...
    protected function fetch(
      string $expression,
      NULL|string|callable|\DOMNode|iterable $filter = NULL,
      NULL|string|callable|\DOMNode|iterable $stopAt = NULL,
      int $options = 0
    ): static {
      return $this->spawn(
     ...
     */
    private function setContentType(string $value): void {
      $mapping = [
        'text/xml' => 'text/xml',
        'xml' => 'text/xml',
    ...
    public function getSelectorCallback(
      callable|string|iterable|\DOMNode $selector = NULL
    ): ?\Closure {
      if (NULL === $selector) {
        return NULL;
      }
      if ($closure = Constraints::filterCallable($selector)) {
        return $closure;
      }
     ...
     * @example interfaces/ArrayAccess.php Usage Example: ArrayAccess Interface
     */
    public function offsetGet(mixed $offset): ?\DOMNode {
    ...
    public function __isset(string $name): bool {
      return match ($name) {
        'contentType', 'length', 'xpath' => TRUE,
        'document' => NULL !== $this->_document,
        default => FALSE,
      };
    }
     ...
     * @throws \InvalidArgumentException
     */
    public function spawn(iterable|\DOMElement|\DOMCharacterData $elements = NULL): static {
      $result = clone $this;
      $result->_parent = $this;
    ...
    public function push(
      iterable|\DOMElement|\DOMCharacterData $elements = NULL, bool $ignoreTextNodes = FALSE
    ): static {
    ...
    public function each(
      callable $function,
      callable|bool $elementsFilter = NULL
    ): self {
     ...
     */
    public function find(mixed $selector, int $options = 0): static {
      [
        $selectorIsScalar,
        $selectorIsFilter,
        $expression,
    ...
    private function prepareFindContext(mixed $selector, int $options): array {
     ...
     * @throws \InvalidArgumentException
     */
    public function index(string|\DOMNode|iterable $selector = NULL): int {
    ...
    public function serializerFactories(
      Serializer\Factory\Group $factories = NULL
    ): Serializer\Factory\Group {
      if (NULL !== $factories) {
